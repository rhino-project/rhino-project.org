...
Rhino uses server side cookies for auth, so you must have [domains that meet the auth rules](../core/auth)
...
Redis is required only for [rhino_jobs](../extensions/jobs) support and rollbar is required for [error trapping](error_reporting).
