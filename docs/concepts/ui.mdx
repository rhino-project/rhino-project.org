...
If the paths are passed as a fucntion, it will receive as arguments the current user's roles and the resources in context. For instance, `paths` related to `create` would receive `null` as `resource`, whereas the `edit` case would lead to `resource` being the current record. For `index`, the list of resources would be passed.
...
These are components that provide a basic context. They are useful for composing more specific components or avoiding global overrides. `ModelIndexTableBase` is an example of a base component.
...
These are components that layout or groups multiple components together, typically the components they compose are [overrideable](#overrides). `FilterLayoutVertical` is an example of a composite component that lays out filters with a vertical form label and a filter field.
