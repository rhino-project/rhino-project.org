...
- `eq`: **equal to**
- `*_coalesce`: **coalescing operators**
- `diff`: **different from**
...
Coalescing operators are used to compare a column against a value and [COALESCE](https://www.postgresql.org/docs/current/functions-conditional.html#FUNCTIONS-COALESCE-NVL-IFNULL) the column to the value if the column is null. This is useful when a row should potentially be included in results if it has a null value. The available coalescing operators are:

- `gt_coalesce`: **greater than**
- `lt_coalesce`: **less than**
- `gteq_coalesce`: **greater than or equal to**
- `lteq_coalesce`: **less than or equal to**
- `eq_coalesce`: **equal to**
...
_Has many_ replationships works the same way as _belong to_. However, it is important to mind the correct name of the relationship, generally this kind of association uses the plural name of the related table. If blog posts had a 1:N relationship with, for example, a comments table, it would be possible to fetch posts that had at least one comment made by a user with id equal to 55:
