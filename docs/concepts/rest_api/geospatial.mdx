...
The `max_distance` parameter is required and is the maximum distance in miles from the location to search for results. Pass units as `km` or change the default in `server/config/initializers/geocoder.rb`.
