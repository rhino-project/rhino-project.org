...
Rhino uses dotenv to load environment variables from `.env` files. The `.env` files are located in the root of the server or client directory. The `.env` file is not checked into source control.

The server uses the [dotenv-rails](https://github.com/bkeepers/dotenv) gem to load the `.env` file. The client uses the built in [Vite env support](https://vitejs.dev/guide/env-and-mode) to load the `.env` file.
...
:::tip
VITE_MY_CUSTOM_ENV can also be added to `client/src/vite-env.d.ts` if you want to use it directly from `import.meta.env`.
:::
