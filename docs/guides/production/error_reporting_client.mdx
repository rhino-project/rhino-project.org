...
Establishing an error reporting strategy demands a good understanding of [React's error boundaries](https://reactjs.org/docs/error-boundaries.html). Error boundaries can catch errors thrown during components rendering, but not the ones thrown in event handlers, like buttons' `onClick` callbacks, and also not unhandled promise rejections. This [section](https://reactjs.org/docs/error-boundaries.html#how-about-event-handlers) explains it.
...
The **only** environment variable **necessary to make Rollbar work** is `VITE_ROLLBAR_ACCESS_TOKEN`, that can be found in the `env.sample` file. By default, Rollbar will not work if there's no access token set, but neither will it break the application.
...
It can be useful to know which user suffered from the error. It's possible to set the user information under Rollbar's context, so it is currently being done in `AuthProvider`:
...
const AuthProvider = ({ children }) => {
  ...
  const { isError, isInitialLoading, isSuccess, data, refetch } = sessionQuery;
