...
ModelIndex: { props: { filter: { published: true } } }
```

:::tip

It is likely you still want to use the base owner filter. You can do this by using the useBaseOwnerFilters hook.

```jsx
const filter = useBaseOwnerFilters('blog', { extraFilters = { published: true } });
```
...
```rhinoconfig title="src/rhino.config.js" levels=global,model
ModelIndex: { props: { order: 'created_at' }}
```
