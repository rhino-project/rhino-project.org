...
## No navigation when saving

By default the save action will navigate to show the new item, if the preferred action is to stay on the same page, use the default save action which provides no navigation.

```rhinoconfig title="src/rhino.config.js" levels=global,model
ModelCreateActions: { props: { overrides: { ModelCreateActionSave: ModelCreateActionSave } } }
```
