...
Docker compose configuration is provided for: `postgres`, `redis`, `backend` and `vite`. All have beend configured with named volumes. Docker containers don't persist data, any data is deleted when the container is removed. The volumes make sure we don't lose any data and speed up starting the containers.
