...
#### 3. Fetch the `2-x-stable` branch from Rhino and name it `boilerplate-2-x-stable` locally:

```
git fetch boilerplate 2-x-stable:boilerplate-2-x-stable
```

#### 4. Set the newly created `boilerplate-2-x-stable` branch to track its upstream branch `2-x-stable`, from Rhino's origin:

```
git branch --set-upstream-to=boilerplate/2-x-stable boilerplate-2-x-stable
```
...
```
git checkout boilerplate-2-x-stable
git pull
```
...
#### 4. Merge the updated `boilerplate-2-x-stable` into the newly created branch:

```
git merge boilerplate-2-x-stable
```
...
Date:   Mon Jan 10 20:29:39 2022 -0300
    ...
    Merge branch 'boilerplate-2-x-stable' into task/NUB-777
```
